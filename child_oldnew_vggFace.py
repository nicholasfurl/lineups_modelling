...
def get_test_distances(Ematrix, face_descriptions, gender, num_fake_subs):
    ...
    for fake_sub in range(num_fake_subs):
        ...
        for idx, row in test_stimuli.iterrows():
            ...
            # Get the distances to all study items using the row/column index
...
def plot_distances_by_study_conditions(test_distances):
    ...
    distances_plot_data = test_distances.groupby(['Study caricature','Test caricature', 'Simulated participant'])['Nearest distance'].mean().reset_index()
    ...
    sns.stripplot(data=distances_plot_data, y = "Nearest distance", x='Study caricature', hue='Test caricature', jitter=True, dodge=True, order = order, hue_order = order)
    #alpha = .4
    
    sns.boxplot(data=distances_plot_data, y = "Nearest distance", x='Study caricature', hue='Test caricature', dodge=True, order = order, hue_order = order, showfliers=False,boxprops=dict(alpha=.3))
